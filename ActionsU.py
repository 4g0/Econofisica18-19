...
def workAndProduce(anHousehold, *argv):
    ...
    firmList = argv[0]
    if common.verbose:
        print(anHousehold, "is searching a job")
    def SearchWork(attempts):
        if attempts<2:
            aFirm = np.random.choice(firmList)
            if aFirm.requestedAmount>0:
                if common.verbose:
                ...
                anHousehold.disposableSavings += aFirm.averagePreviousPriceForGoods
                anHousehold.employed = True
                if common.verbose:
                    print("\t", aFirm, "stockpile was", aFirm.stockpile, "and now is ", aFirm.stockpile + 1)
                    print("\t", aFirm, "amount was", aFirm.requestedAmount, "and now is ", aFirm.requestedAmount - 1)
                aFirm.stockpile += 1 #1 is the quantity produced
                aFirm.requestedAmount -= 1
                return None
            else:
                if common.verbose:
                    print(anHousehold," is searching another job after ",attempts,"attempts")

                SearchWork(attempts + 1)
        else:
            if common.verbose:
                print("\t unemployment was", unp.unemployment, "and now is ", unp.unemployment + 1)
            anHousehold.employed = False
            return None
...
def buyGood(anHousehold, *argv):
    ...
    if (anHousehold.employed == True) or (anHousehold.employed == False and randomValue < common.unemploymentConsumptionProbability): 
        TryToBuy(0)
...
def buyBond(anHousehold, *argv):
    ...
    randomValue = np.random.random()
    if anHousehold.disposableSavings >= common.initialBondPrices:
        if common.currentInterestRate == "Low":
            if randomValue < common.lowerProbabilityBoundBasedOnInterestRate:
                ...
                if common.verbose:
                    ...
                    print("\t\tHis disposable income was", anHousehold.disposableSavings)
                anHousehold.disposableSavings -= common.initialBondPrices
                anHousehold.bondAmount +=1
                if common.verbose:
                    print("\t\tand now is ", anHousehold.disposableSavings)
            else:
                if common.verbose:
...
def interactWithStockMarket(anHousehold, *argv):
    ...
    # here interacts with the market
    bookList = argv[1].copy()
        ...
        if action == "Ask" and anHousehold.stockForSharesOfFirms[indexOfCurrentInteraction] > 0:
            ...
            if matched:
                ...
                if common.verbose:
                    ...
                    print("\t\t", interactingHousehold, "disposable income was",interactingHousehold.disposableSavings)
                interactingHousehold.disposableSavings -= interactingPrice
                if common.verbose:
                    print("\t\tNow it is ",interactingHousehold.disposableSavings)
                    print("\t\t", anHousehold, "disposable income was",anHousehold.disposableSavings)
                anHousehold.disposableSavings += interactingPrice
                if common.verbose:
